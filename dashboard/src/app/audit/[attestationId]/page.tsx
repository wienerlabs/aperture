...
import { complianceApi, type Attestation } from '@/lib/api';
...
export default function AuditPage() {
  ...
  const fetchAttestation = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await complianceApi.getAttestation(attestationId);
      if (response.data) {
        ...
        {!loading && error && (
          <div className="w-full max-w-lg">
            <div className="bg-[rgba(20,14,0,0.8)] backdrop-blur-md border border-red-400/20 rounded-xl p-8 text-center">
              ...
              <p className="text-amber-100/40 text-sm">
                The requested attestation could not be retrieved. It may not exist
                or may have expired.
              </p>
              <p className="text-amber-100/20 text-xs mt-4 font-mono">
                ID: {truncateAddress(attestationId, 8)}
              </p>
            </div>
          </div>
        )}
          ...
          <div className="w-full max-w-lg">
            ...
            <div className="flex justify-center mb-6">
              <div className="flex items-center gap-3 px-8 py-4 rounded-2xl bg-green-500/10 border-2 border-green-400/40">
                ...
                <span className="text-3xl font-black text-green-400 tracking-wider">
                  COMPLIANT
                </span>
              </div>
            </div>
            ...
            <div className="bg-[rgba(20,14,0,0.8)] backdrop-blur-md border border-amber-400/20 rounded-xl overflow-hidden">
              ...
              <div className="px-8 py-6 space-y-4">
                <div className="flex justify-between items-start">
                  <span className="text-sm text-amber-100/40">Operator</span>
                ...
                <div className="flex justify-between items-start">
                  <span className="text-sm text-amber-100/40">Period Start</span>
                  <span className="text-sm text-amber-100">
                    {formatDate(attestation.period_start)}
                  </span>
                </div>

                <div className="flex justify-between items-start">
                  <span className="text-sm text-amber-100/40">Period End</span>
                  <span className="text-sm text-amber-100">
                    {formatDate(attestation.period_end)}
                  </span>
                </div>
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-sm font-mono text-amber-100 font-medium">
                    {attestation.total_payments}
                  </span>
                </div>
                ...
                <div>
                  <span className="text-sm text-amber-100/40 block mb-1">
                    Proof Hash
                  </span>
              ...
              <div className="mx-8 mb-6 p-4 rounded-lg bg-amber-400/5 border border-amber-400/10">
                ...
                <p className="text-xs text-amber-100/40">
                  ...
                  Individual payment amounts, recipients, and sender identities
                  remain private.
                </p>
              </div>
              ...
              <div className="px-8 pb-8">
                {txSignature ? (
                  ...
                  <div className="flex items-center justify-center gap-2 w-full bg-amber-500/30 text-amber-100/50 font-bold rounded-lg px-6 py-3 cursor-not-allowed">
                    <ExternalLink className="w-4 h-4" />
            ...
            {/* Footer */}
            <p className="text-center text-amber-100/20 text-xs mt-6">
